 ...
 */
 ...
 void app_thread_entry(uint32_t data){
    ...
    */
     ...
     kalman_init(&kf_0xc91a, 0.005, 15.0, 500.0); //3
     kalman_init(&kf_0x0e0d, 0.005, 15.0, 500.0); //1
     kalman_init(&kf_0x4795, 0.005, 15.0, 500.0);  //4
     kalman_init(&kf_0xcd9e, 0.005, 15.0, 500.0); //2
     //cau hinh
         ...
         if (i < loc.anchors.an_pos.cnt) {
           ...
           // ?p d?ng Kalman Filter
                     ...
                     switch(loc.anchors.dist.addr[i]) {
                         case 0xc91a:
                             filtered_dist = kalman_update(&kf_0xc91a, loc.anchors.dist.dist[i]);
                             break;
                         case 0x0e0d:
                         ...
                         case 0x4795:
                             ...
                             break;
                         case 0xcd9e:
                             filtered_dist = adaptive_kalman_update(&kf_0xcd9e, loc.anchors.dist.dist[i], loc.anchors.dist.qf[i]);
                             break;
                         default:
